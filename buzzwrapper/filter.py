...
class Filter(Monitor):
    def __init__(self, id=None, monitor_id=None, title=None, keywords=None):
        ...
        if id is None and all([monitor_id, title, keywords]):
            self.id = Filter.add(
    ...
    def add(monitor_id, title, keywords):
        """
        Adds filter with keywords and title to a buzz monitor specified by id
        and returns filter id.
        """

        url = (
            "https://forsight.crimsonhexagon.com/ch/monitor/"
            + str(monitor_id)
            + "/filter"
        )
        params = {
    ...
    def delete(self):
        """
        Deletes filter specified by monitor and filter id and returns response
        dict whether action was successful.
        """
